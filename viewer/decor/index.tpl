...
if(!isset($_GET['type']) or $_GET['type'] == null)
    header('location: ' . URL . '?id=' . security::clean($_GET['id']) . '&go=decor&type=general_decor');
    ?>
    <aside id="left">
        <article>
            <ul id="left_menu">
                <li><a href="<?= URL ?>?go=decor&type=general_decor"><img src="img/bullet.png">General Decorations</a><p></p></li>
            </ul>
        </article>                   
    </aside>
    <div id="bodyOrignal">
        <div class="row">
            ...
            switch($_GET['type']){
                case "general_decor":
					$data = $Database->SelectW('store', array("type" => "item_decor", "enable" => 1) );
                    break;
                    
                default :
                    header('location: ' . URL . '?go=decor&type=general_decor');
            }
			while ($row = array_shift($data)) {
				...
				<div class="col-lg-4">
					...
					<p><a class="btn btn-default" href="<?= URL ?>?go=decor&iditem=<?= md5($row['id'] . date('Y-m-d')) ?>" role="button">View details »</a></p>
				</div>
			<?php } ?>
        </div>
    </div>
